      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <Link to="/" className="flex items-center space-x-3 group">
            <img src="/logo.png" alt="Logo" className="h-9 w-auto transition-transform duration-300 group-hover:scale-105" />
          </Link>

          {/* Navigation Links - Only show on larger screens */}
          <div className="hidden md:flex items-center space-x-6">
            <Link to="/" className={`text-base font-medium transition-colors duration-200 relative py-2 ${isActive("/") ? "text-white after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-blue-500" : "text-zinc-400 hover:text-white"}`}>
              Home
            </Link>
            <Link to="/blogs" className={`text-base font-medium transition-colors duration-200 relative py-2 ${isActive("/blogs") ? "text-white after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-blue-500" : "text-zinc-400 hover:text-white"}`}>
              Blogs
            </Link>
            ...
            {session && <Link to="/admin">
                <Button variant="outline" size="sm" className="ml-2 bg-zinc-800 hover:bg-zinc-700">
                  Dashboard
                </Button>
              </Link>}
            
            {/* Login/Logout Button */}
            {!session ? <Link to="/login">
                <Button size="sm" className="bg-blue-600 hover:bg-blue-700">
                  Sign In
                </Button>
              </Link> : <Link to="/login">
                <Button variant="outline" size="sm" className="border-zinc-700 bg-zinc-300 hover:bg-zinc-200 text-zinc-800">
                  Sign Out
                </Button>
              </Link>}
          </div>
          ...
          <div className="md:hidden flex items-center">
            <button type="button" className="text-zinc-300 hover:text-white focus:outline-none" onClick={toggleMenu}>
              {isMenuOpen ? <X className="h-6 w-6 transition-transform duration-200" /> : <Menu className="h-6 w-6 transition-transform duration-200" />}
            </button>
          </div>
        </div>
