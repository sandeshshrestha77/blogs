...
const CommentForm = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Note: In a real application, you would have a comments table
  ...
  post: Post;
    ...
    <div className="flex gap-4 items-start">
      <div className="w-20 h-20 rounded-md overflow-hidden flex-shrink-0">
        <img src={post.image || "https://source.unsplash.com/100x100/?technology"} alt={post.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" />
      </div>
      <div>
...
const BlogPost = () => {
  ...
  const fetchRelatedContent = useCallback(async () => {
    try {
      ...
      // In a real app, you would fetch real comments from a comments table
      // For demo purposes, we'll generate some sample comments
      const sampleComments: Comment[] = [{
        id: '1',
        name: 'John Doe',
        email: 'john@example.com',
        content: 'Great article! I learned a lot from your insights. Looking forward to more content like this.',
        created_at: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString() // 7 days ago
      }, {
        id: '2',
        name: 'Jane Smith',
        email: 'jane@example.com',
        content: 'I appreciate how you broke down complex concepts into easy-to-understand pieces. This is exactly what I was looking for!',
  ...
  const handleCommentAdded = () => {
    // In a real app, you would refresh comments from the database
    // For demo purposes, we'll add a new comment to the top of the list
    const newComment: Comment = {
      id: `new-${Date.now()}`,
      name: 'New Commenter',
      email: '',
      content: 'This is a newly added comment. In a real application, this would come from the database.',
