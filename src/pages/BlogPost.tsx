...
const BlogPost = () => {
  ...
  const formatContent = (content: string) => {
    ...
    return content.split('\n\n').map((paragraph, index) => <p key={index} className="mb-6 text-gray-700 leading-relaxed">
        {paragraph}
      </p>);
  };
  if (loading) {
  ...
  if (!post) {
    return <div className="min-h-screen flex flex-col bg-zinc-200">
        <Navbar />
      ...
      <article className="container mx-auto px-4 py-16 max-w-6xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            {post.category && <span className="inline-block text-xs font-medium px-2 py-1 bg-primary/10 text-primary rounded-full mb-4">
                {post.category}
              </span>}
            ...
            <div className="aspect-video mb-8 rounded-xl overflow-hidden shadow-md">
              <img src={post.image || "https://images.unsplash.com/photo-1498050108023-c5249f4df085"} alt={post.title} className="object-cover w-full h-full transition-transform hover:scale-105" loading="lazy" />
            </div>
            <div className="prose prose-lg max-w-none">
              {post.content ? formatContent(post.content) : <p className="text-gray-700">No content available.</p>}
            </div>
          </div>
          <aside>
