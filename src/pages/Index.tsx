...
const Index = () => {
  ...
  useEffect(() => {
    const fetchFeaturedPost = async () => {
      try {
        const {
          data,
          error
        } = await supabase.from("posts").select().eq('featured', true).maybeSingle();
    ...
    fetchFeaturedPost();
    const channel = supabase.channel('featured-post-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'posts',
      filter: 'featured=eq.true'
    }, () => fetchFeaturedPost()).subscribe();
    return () => {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="relative animate-fade-in">
              <div className="rounded-2xl overflow-hidden border border-zinc-800/50 shadow-xl transition-all duration-500 hover:shadow-blue-500/10 hover:border-blue-500/20">
                <img alt="About" className="w-full h-auto hover:scale-105 transition-transform duration-700" src="/lovable-uploads/26e396a9-608b-4f0e-9650-c7598d805737.jpg" />
              </div>
            </div>
          ...
          <div className="container mx-auto px-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-12">
              ...
              <div className="flex gap-3 animate-fade-in">
                {!showAllPosts && displayedPosts.length > 6 && <Button variant="outline" onClick={handleViewAll} className="border-blue-600/40 text-blue-400 hover:bg-blue-600/10 hover:text-blue-300 transition-all">
                    View More
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {displayedPosts.map((post, index) => <div key={post.id} className="animate-fade-in" style={{
            animationDelay: `${index * 0.1}s`
          }}>
                  <BlogCard title={post.title} excerpt={post.excerpt} image={post.image} author={post.author} date={post.date} categories={post.category ? [post.category] : []} slug={post.slug} read_time={post.read_time} alt_text={post.alt_text} meta_title={post.meta_title} meta_description={post.meta_description} views={post.views} />
                </div>)}
            </div>
          </div>
        </section>}

      {!updating && !featuredPost && (!displayedPosts || displayedPosts.length === 0) && <div className="text-center py-40">
