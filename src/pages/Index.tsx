...
const Index = () => {
  ...
  return <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-24">
        {loading ? <div className="flex items-center justify-center min-h-[40vh]">
            <LoadingSpinner />
          </div> : <>
            {/* Featured Post */}
                ...
                <div className="bg-card rounded-xl overflow-hidden shadow-xl hover:shadow-2xl transition-shadow duration-300">
                  <div className="grid md:grid-cols-2 gap-0">
                    <div className="aspect-w-16 aspect-h-9 md:aspect-h-full">
                      <img src={featuredPost.image || 'https://images.unsplash.com/photo-1498050108023-c5249f4df085'} alt={featuredPost.title} className="object-cover w-full h-full transition-transform hover:scale-105" />
                    </div>
                    <div className="p-12 flex flex-col justify-center">
                      <div className="flex gap-2 mb-4">
                        {featuredPost.category && <span className="text-xs font-medium px-3 py-1 bg-primary/10 rounded-full text-gray-950">
                      ...
                      <Button size="lg" onClick={() => navigate(`/blog/${featuredPost.slug}`)} className="self-start text-primary-foreground bg-black">
                        Read More
                      </Button>
                    </div>
                  </div>
            ...
            {posts.length > 0 && <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {posts.map(post => <BlogCard key={post.slug} {...post} categories={post.category ? [post.category] : []} />)}
                </div>
              </div>}

            {!loading && !featuredPost && posts.length === 0 && <div className="text-center py-12">
